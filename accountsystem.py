...
accountsystem.title('Sistema di Account')
...
def signup():
    if FirstName.get() == "" or LastName.get() == "" or Password.get() == "" or Email.get() == "" or ConfirmPassword.get() == "":
        ...
        messagebox.showerror("ERRORE", "LA PASSWORD E LA CONFERMA DELLA PASSWORD NON COINCIDONO")
        ...
        try:
            ...
            connection.close()
            clear()
            messagebox.showinfo('SUCCESSO', "NUOVO ACCOUNT CREATO CON SUCCESSO")

        except Exception as es:
            messagebox.showerror("ERRORE", 'QUALCOSA È ANDATO STORTO, RIPROVA')
...
def login():
    ...
    if result:
        ...
        messagebox.showerror("FALLITO", "SPIACENTI, NON È POSSIBILE ACCEDERE. SI PREGA DI RIPROVARE")
...
def forgot_password():
    ...
    def change_password():
        if email_entry3.get() == "" or new_password_entry.get() == "":
            ...
            row = cursor.fetchone()
            if row == None :
                messagebox.showerror("ERRORE", "L'INDIRIZZO EMAIL NON ESISTE")
                exit_window()
                ...
                db.commit()
                db.close()
                messagebox.showinfo('CONGRATULAZIONI', "CAMBIAMENTO PASSWORD EFFETTUATO CON SUCCESSO")
                exit_window()
